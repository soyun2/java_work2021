...
public class MemberDao {
	...
	public int findByUsernameAndPassword(String username, String password) {
		...
		try {
			...
			if (rs.next()) { // next()함수는 커서를 한칸 내리면서 해당 행에 데이터가 있으면 true, 없으면
								// false 반환
	...
	// 성공 1, 실패 -1		회원가입한 정보를 DB에 저장함
	public int save(Member member) {
		conn = DB.getConnection();

		try {
			pstmt = conn.prepareStatement("insert into member values(?,?,?,?,MEM_NUM.NEXTVAL)");
